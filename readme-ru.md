...
## Сокрытие реального имени комментируемой модели и/или реального `id` записи модели
По умолчанию, библиотека будет использовать в формах добавления и получения комментария реальные имена класса модели (например: `App\BlogPost`) и реальный `id` из базы данных. Это может быть нежелательно.

### Переопределение реального названия класса модели
Вы можете переопределить реальное имя класса модели в формах, используя настройки (`config/comments.php`) по следующему принципу:
```
    'rewriteModel' => [
        '\App\BlogPost' => 'FakeModelName',
    ],
``` 
Где `\App\BlogPost` это реальное название класса (не забудьте \ в начале), а `FakeModelName` будет фигурировать в формах.

### Переопределение использования реального `id` записи модели
Если вы используете какие-либо способы сокрытия реального `id` ваших записей (например: [Optimus](https://github.com/cybercog/laravel-optimus)), вы можете использовать такой же способ и в настоящей библиотеке. Для этого вам необходимо создать в своей модели метод для [виртуального свойства](https://laravel.com/docs/master/eloquent-mutators#defining-an-accessor), который будет возвращать закодированный `id`, и метод, который будет возвращать запись по ранее закодированному `id`.
...
#...
	/**
	* Получение записи по закодированному $id
	*
	* @param $id int 
	* @return \Illuminate\Database\Eloquent\Model
	*/
	public static function decryptFindOrFail($id)
	{
		//здесь ваш способ расшифровки $id
		//..
	...
	/**
	* Возвращает закодированный $id записи
	*
	* @return int
	*/
	...
	{
		$id = $this->id;
		//здесь ваш способ КОДИРОВАНИЯ $id
		//..
...
Конфиг 	`config/comments.php`:
```
#...
    
    //указываем название виртуального свойства	
    'rewriteIdAttribute' => [
        '\App\Post' => 'cryptId',
    ],
    
    //указываем название метода
    'rewriteFindMethod' => [
        '\App\Post' => 'decryptFindOrFail'
    ],    
#...
```
